...
if (isset($_POST) && isset($_POST["total"])) {
    ...
    if(is_numeric($order_total)) {
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            // Kiểm tra xem có đặt lịch nào chưa, nếu có thì cập nhật order_id vào bảng datlich
            if (isset($order_id) && is_numeric($order_id)) {
                // Cập nhật order_id vào bảng datlich cho bản ghi có id trùng với order_id vừa tạo
                $update_sql = "UPDATE datlich SET id = {$order_id} WHERE id IS NULL";
        ...
        <div class="col-md-8 mx-auto">
            ...
            <?php if (!is_numeric($order_id)) { ?>
                ...
                <form method="POST" action="">
                    <p>Quý phụ huynh cần phải thanh toán số tiền là 299.000đ để có thể đặt lịch hẹn tư vấn với chuyên viên tại trường!</p>
                    <div class="mb-3">
                        <label for="amountInput" class="form-label">Số tiền</label>
                        <input type="number" name="total" class="form-control" id="amountInput" aria-describedby="emailHelp" placeholder="Nhập số tiền">
                        <div id="amountInputHelp" class="form-text">Vui lòng điền số tiền</div>
                    </div>

                    <button type="submit" class="btn btn-primary" name="orderC">Đặt</button>
                </form>
            <?php } else { ?>
                <!-- Form tạo đơn hàng -->
                ...
                $result = $conn->query("SELECT * FROM tb_orders where 
                id={$order_id}");
                if ($result) {
